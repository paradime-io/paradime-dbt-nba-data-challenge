...
season_inflation as (
    select
        ...
        (_1800_dollar_value + _1800_dollar_value / (1 + inflation_vs_previous_year)) / 2 as _1800_dollar_value
...
inflation_adj_salaries as (
    select
        season_year_split.*,
        (round((_2024_value._1800_dollar_value / season_inflation._1800_dollar_value) * salary_usd))::int as _2024_adjusted_salary
    from season_year_split
        inner join season_inflation using (season_start_year, season_end_year)
        full join _2024_value
)

select * from inflation_adj_salaries