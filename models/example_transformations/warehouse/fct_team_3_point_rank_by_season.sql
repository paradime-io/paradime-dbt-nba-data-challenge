...
where three_point_rank between 1 and 10 group by team_name, season, three_point_rank, three_pointers_attempted 
order by season asc, three_point_rank asc)
--the above CTE produces the top ten ranked teams per season by three pointers attempted and the number of three pointers attempted by each team
select * from final