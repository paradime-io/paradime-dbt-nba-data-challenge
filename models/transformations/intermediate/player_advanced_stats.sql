with player_game_logs as (
    select 
        ...
        round(div0(field_goals_made, field_goals_attempted), 3) as field_goal_pct,
        three_point_made as three_point_made,
        three_point_attempted as three_point_attempted,
        round(div0(three_point_made, three_point_attempted), 3) as three_point_pct,
        free_throws_made as free_throws_made,
        free_throws_attempted as free_throws_attempted,
        round(div0(free_throws_made , free_throws_attempted), 3) as free_throw_pct,
        offensive_rebounds,
        defensive_rebounds,
...
select 
    *,
    round(DIV0(field_goals_made + 0.5 * three_point_made, field_goals_attempted), 3) as effective_field_goal_percentage,
    round(div0(0.5 * points, field_goals_attempted + 0.44 * free_throws_attempted), 3) as true_shooting_percentage,
    ...
    round(div0(100 * (offensive_rebounds + defensive_rebounds) * (team_minutes_played / 5), (mins_played * (team_total_rebounds + opponent_total_rebound))),3) as rebound_percentage,
    --Steal Percentage=100*(Steals*(Team Minuted Played/5))/(Minuted Played*Opponent’s Possessions)
    round(div0(100 * steals * (team_minutes_played / 5), mins_played * opponent_possessions), 3) as steal_percentage,
    --BLK% - Block Percentage: 100 * (BLK * (Tm MP / 5)) / (MP * (Opp FGA - Opp 3PA))
    round(div0(100 * blocks * (team_minutes_played / 5), mins_played * (opponent_field_goals_attempted - opponent_three_point_attempted)),3) as block_percentage,
    --simplified PER
    round(div0((field_goals_made * 85.91 + steals * 53.897 + three_point_made * 46.845 + blocks * 39.190 + offensive_rebounds * 39.19 + assists * 34.677 + defensive_rebounds * 14.707 + personal_fouls * 17.174 + (free_throws_attempted - free_throws_made) * 20.091 + (field_goals_attempted - field_goals_made) * 39.190 + turnovers * 53.897) , mins_played), 3) as PER
